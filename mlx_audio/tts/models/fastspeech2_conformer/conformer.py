...
class FastSpeech2ConformerConfig:
    ...
    speech_decoder_postnet_units: int = 384
...
def length_regulator(encoded_embeddings, duration_labels, speaking_speed=1.0):
    ...
    for i, (encoded_embedding, target_duration) in enumerate(zip(encoded_embeddings, duration_labels)):
        indices = mx.arange(len(target_duration))
        # Convert target_duration to a list of integers
        target_duration_list = [int(d) for d in target_duration]
        # Use mx.repeat with a single integer for each token position
        expanded_indices = mx.concatenate([mx.repeat(mx.array([i]), d) for i, d in enumerate(target_duration_list)])
        # Use the expanded indices to gather the corresponding embeddings
        repeated = encoded_embedding[expanded_indices]
...
class FastSpeech2ConformerConvolutionModule(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = nn.GLU()(hidden_states, dim=1)
...
class ConformerModel(FastSpeech2ConformerPreTrainedModel):
    """
    FastSpeech 2 module.
