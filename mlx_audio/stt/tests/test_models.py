...
class TestWhisperModel(unittest.TestCase):
    ...
    def test_from_pretrained(
        ...
        mock_open.return_value.__enter__.return_value.read.return_value = json.dumps(dummy_config)
        ...
        model_instance = Model.from_pretrained(path_or_hf_repo="mlx-community/whisper-tiny", dtype=mx.float32)

        self.assertIsInstance(model_instance, Model)
        self.assertEqual(model_instance.dims.n_mels, dummy_config["n_mels"])
        mock_snapshot_download.assert_called_once_with(repo_id="mlx-community/whisper-tiny")
    ...
    def test_generate_simple_case(
        ...
        def actual_decode_side_effect(tokens_to_decode):
            ...
            for token_val in tokens_to_decode:
                t = int(token_val)
                if t == 100: text_parts.append("hello")
                elif t == 200: text_parts.append("world")
                elif t == EOT_TOKEN_ID: break
        ...
        mock_decoding_result = DecodingResult(
            ...
            temperature=0.0, avg_logprob=-0.25, compression_ratio=1.2, no_speech_prob=0.05,

            audio_features=mx.zeros((1, self.dims.n_mels), dtype=mx.float32), language="en"
        )
        ...
        with patch.object(real_model_for_test, 'decode', return_value=mock_decoding_result) as mock_instance_decode:
            output = real_model_for_test.generate(
                dummy_audio_input, language="en", word_timestamps=False, temperature=0.0, fp16=False
            )

            mock_instance_decode.assert_called_once()
            args_decode_call, _ = mock_instance_decode.call_args
            self.assertEqual(args_decode_call[0].shape, (N_FRAMES, self.dims.n_mels)) # mel_segment
        ...
        self.assertAlmostEqual(segment["compression_ratio"], mock_decoding_result.compression_ratio)
        self.assertAlmostEqual(segment["no_speech_prob"], mock_decoding_result.no_speech_prob)

        mock_log_mel.assert_called_once_with(ANY, n_mels=self.dims.n_mels, padding=N_SAMPLES)
